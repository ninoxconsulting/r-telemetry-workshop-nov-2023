...
area of interest (a bounding box) around it, so we can use that to subset the 
covariate data we will be using.
...
```{r}
#| eval: false
st_write(scott_aoi, "clean_data/scott_aoi.gpkg", append = FALSE)
```
...
st_write(roads, "clean_data/roads.gpkg", append = FALSE)
```
