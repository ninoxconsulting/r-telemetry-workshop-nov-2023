...
The number of fixes are relatively steady throughout the years for all individuals except SC_car171? something looks strange here as there are large spikes in October 2015 and February 2016.
...
We can again convert to a sf object and export. Note this is required as we previously remove the spatial components to undertake the summaries above.

```{r write-standard-subset-gpk}
sbou_sub_sf <- read_sf(sbou_sub, coords = c("location.long", "location.lat"), 
                        crs = 4326, remove = FALSE) |> 
  st_transform(3005)

```

Now we can export as .gpkg
