...
ref_raw <- readr::read_csv("raw_data/Mountain caribou in British Columbia-reference-data.csv")
...
all_data <- all_data %>% 
  dplyr::select(event.id, location.long, location.lat, DOP, FixType, herd ,
...
bou <- st_as_sf(fdata, coords = c("location.long", "location.lat"), crs = 3005, remove = FALSE)
...
sbou <- bou %>% 
  dplyr::filter(herd == "Scott")

# tidy the data by removing columns that are redundant
sbou <- sbou %>% dplyr::select(-herd, -tag.local.identifier, -`tag-manufacturer-name` , -`tag-model`)
...
range(as.numeric(sbou_dur$diff), na.rm = TRUE)

# most fall in the less than than 10 
hist(as.numeric(sbou_dur$diff))
...
sbou_dur <- sbou_dur %>%
  mutate(date2=as.Date(date_time, format = '%Y-%m-%d')) %>%
  mutate(jdate=julian(date2))
...
write.csv(sbou_sub, "clean_data/scott_herd_subset.csv", row.names = F)
...
# 
# # export as .gpkg
st_write(sbou_sub_sf, "clean_data/scott_herd_subset.gpkg", append = FALSE)
